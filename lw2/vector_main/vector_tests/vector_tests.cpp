...
SCENARIO("Test ReadVector function")
{
	...
	WHEN("Input stream has 1 or more than 1 number")
	{
		std::vector<double> numbers;
...
SCENARIO("Test PrintVector function")
{
	...
	WHEN("Vector has 1 or more than 1 number")
	{
...
SCENARIO("Test MultiplyingVector function")
{
	...
	WHEN("Vector has only positive numbers")
	{
		std::vector<double> numbers = { 1.2, 2.3, 0.5 };
		std::vector<double> result = { 1.2, 2.3, 0.5 };
		THEN("Result vector has the same number")
		{
			MultiplyingVector(numbers);
			CHECK(result == numbers);
		}
	...
	WHEN("Vector has negative numbers")
	{
	...
	WHEN("Vector has positive and negative numbers")
	{
